...
import { useDismissOpportunityAlert } from '../../hooks/useRecommendations';
...
export default function OpportunityAlerts({ alerts }: OpportunityAlertsProps) {
  ...
  const { mutate: dismissAlert } = useDismissOpportunityAlert();

  const visible = alerts.filter(a => !dismissedLocal.has(a.id));
      ...
      <AnimatePresence>
        {visible.map(alert => (
          ...
          >
            ...
            <button
              onClick={() => {
                setDismissedLocal(prev => new Set(prev).add(alert.id));
